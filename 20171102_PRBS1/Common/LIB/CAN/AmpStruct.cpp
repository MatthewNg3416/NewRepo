...
AmpIoCfg::AmpIoCfg( void )
{
   ...
   for (int i=0; i<COPLEY_MAX_INPUTS; i++ )
   {
      inCfg[i] = (INPUT_PIN_CONFIG)0;
      inDebounce[i] = 0;
   }

   for (int i=0; i<COPLEY_MAX_OUTPUTS; i++ )
   {
...
const Error *Amp::GetIoConfig( AmpIoCfg &cfg )
{
   ...
   for (int i=0; i<cfg.inputCt && !err; i++ )
   {
   ...
   for (int i=0; i<cfg.outputCt && !err; i++ )
